from detail import spy, user_detail, Chat, friends_detail
from steganography.steganography import Steganography
from datetime import datetime

OLD_STATUS_MESSAGES = ['hello everyone', 'how do you do', 'every time happy']


print ("welcome to spy chat")

menu_choices = ("Do you want to continue as " + spy.salutation + " " + spy.name + " (Y/N)? ")
...
def add_status():
    ...
    if default.upper() == "N":
        ...
        if len(new_status_message) > 0:
            OLD_STATUS_MESSAGES.append(new_status_message)
        ...
        for message in OLD_STATUS_MESSAGES:
        ...
        if len(OLD_STATUS_MESSAGES) >= message_selection:
            updated_status_message = OLD_STATUS_MESSAGES[message_selection - 1]
...
def add_friend():
    ...
    if len(new_friend.name) > 0 and new_friend.age > 12 and new_friend.rating >= spy.rating:
        friends_detail.append(new_friend)
        print ('Friend Added!')
    else:
        print ('Sorry! Invalid entry. We can\'t add spy with the details you provided')

    return len(friends_detail)
